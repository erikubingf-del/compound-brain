...
| 9 | Background Agents Auto-PR on Worktree Completion | Background agents launched from `claude agents` now commit, push, and open a draft PR when they finish code work in a worktree, instead of stopping to ask. Changes autonomous coding workflow defaults. | https://code.claude.com/docs/en/whats-new/2026-w27 | WATCH — [VERIFIED: official docs w27] | high | REVISED (2026-08-24) — w32 (v2.1.220) changes behavior: background sessions commit and push, but open a draft PR **only when the task calls for one**, not automatically for all code work. Also: the session follows git instructions in CLAUDE.md. |
